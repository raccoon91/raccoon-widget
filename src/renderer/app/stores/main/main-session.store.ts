...
import { useMainStorageStore } from "./main-storage.store";
...
export const useMainSessionStore = create<MainSessionStore>()(
  devtools(
    persist(
      (set, get) => ({
        ...
        pullDeviceInfo: () => {
          ...
          const bluetooth = useMainStorageStore.getState().bluetooth;
      ...
      }),
      {
        name: "session-store",
        skipHydration: true,
        storage: createJSONStorage(() => mainSession),
        partialize: (state) => ({
